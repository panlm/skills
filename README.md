...
| [aws-rightsizing](./aws-rightsizing/) | Right-size an AWS account from measured CloudWatch utilization rather than guesswork. **Read-only** (`describe*`/`list*`/`get*` only, no billing APIs — on-demand pricing is read from the public price list). Covers EC2/EBS/VPC, RDS, ElastiCache, MSK and EKS node-level bin-packing; classifies each resource into downsize / idle / stop-schedule candidates with two selectable `sizing_profile` presets (`aggressive`/`conservative`). All verdict logic lives in a single deterministic `references/core.py` — including first-pass target instance classes for RDS / ElastiCache / MSK, flagged for human review — and is guarded by a per-file regression gate over two desensitized real-fleet baselines. A customer-facing walkthrough of how each recommendation is derived (per-service chapters, formulas, veto ordering, interactive calculator) lives in [`docs/method-explained.html`](./aws-rightsizing/docs/method-explained.html). |
