...
11. 托管服务目标明细  RDS / ElastiCache / MSK 的初选目标与月省，**已计入头条**，此处给明细与依据
```
...
这与本文件对托管服务明细的要求是同一条规则：**并列、各标口径、不相加。**
...
## 托管服务的目标机型（进头条，须标 confidence）

三条托管判据**产出初选目标**：`nonburst` / `burst` 与对应
`nb_save_mo` / `b_save_mo` / `nb_delta_*` / `b_delta_*`。因此它们的节省额
与 EC2 行同等进入路线一/二，**不需要第三条口径** ——
那两条口径本来就是对 `nb_save_mo` / `b_save_mo` 求和，托管行不再恒空而已。

**这条取代了「托管节省结构性不进头条」那条旧裁定。** 旧裁定不是独立选择，
它是「判据不选目标 ⇒ 两列恒空 ⇒ 求和为 0」的下游后果；一旦判据选目标，
把它排除在外反而是漏算。`other_save_mo` 仍然留空，理由不变：那一列是给
**采集侧**已确定目标的变更用的（`gp2→gp3`、节点池 bin-packing），
托管目标由 `core.py` 选，走 `nb_save_mo` / `b_save_mo` 正路。

代价与约束，逐条都要落到报告里：

1. 托管行 `verdict` 仍是 `downsize-candidate`，且**必带**一条 blocker
   「目标实例类 / node type 为本 skill 初选……须人工确认变更窗口与回滚预案」。
   人工的角色是**审核初选**，不是选型。
2. 托管行 `confidence` 只有 `low`（`0 < sample_n < min_biz_hours_points` 时）
   或**留空**，**永不 `medium` / `high`** —— 与本文件派生列清单里那句
   「托管判据行留空」是同一条规则的两半：`high`/`medium` 在 EC2 侧的含义是
   「CPU 与内存两轴都有/没有实测数据」，托管侧没有那个轴，套用等于自造语义；
   而 `low` 只需要采样轴，托管侧有，所以它会出现。摘要必须给出
   `confidence=low` 的 breakout —— 这条规则此前对托管行**永不触发**，
   因为那一列恒空；现在低样本的托管行会被它捞出来。
   （归档回放实测：31 条托管行里 23 条留空、8 条 `low`，`medium` 一次没出现。）
3. 摘要须单列一行「其中托管服务（目标待人工确认）$X」。**这不是第三条口径**，
   是同一个分子的成分拆分，让读者能把它从 EC2 侧的金额里分出来。
4. 候选池为空时，报告必须写出**是哪条约束绑定**（五种成因：无更便宜候选 /
   全部跨架构 / 需求量超候选 / 内存装不下 / 被降幅地板挡住）。
   给同一句笼统话是缺陷 —— 那句话在后三种成因下是错的。

本节仍单列，逐条给出：资源 ID、当前规格与单价、目标与单价、月省、
`required_vcpu` / `required_gib`（或 `required_gib_usable`）、`blockers`。
与第 1 节的关系从「不计入」改为**「已计入，此处给明细」**。
