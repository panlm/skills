...
1. 托管行 `verdict` 仍是 `downsize-candidate`，且**必带**一条 blocker
   ...
   人工的角色是**审核初选**，不是选型。
2. 托管行 `confidence` 只有 `medium` / `low`，**永不 `high`**。`high` 在 EC2 侧
   的含义是「CPU 与内存两轴都有实测数据」，语义不同，不可套用；托管侧的适配
   校验建在观测稳态上，不含业务增长输入。摘要必须给出 `confidence=low` 的
   breakout —— 这条规则此前对托管行**永不触发**，因为那一列是空的。
