...
Defines the directory layout and README.md template produced by this skill.
All placeholders use `{CURLY_BRACES}` notation.

## Contents

- Directory Layout
- Slug Naming Conventions (scenario-slug, target-slug, context-slug)
- README.md Template
...
**scenario-slug:**
- Use the abbreviation table in `references/slug-conventions.md`
...
- Downstream service or purpose that distinguishes experiments with the same
  scenario + target
- Lowercase, hyphens only, truncated to max 10 characters
- Used for network fault injection actions (latency, packet-loss, blackhole-port)
  to identify the target downstream service (e.g., `redis`, `msk`, `dynamo`)
- Omit for non-directional actions (pod-delete, cpu-stress, memory-stress, etc.)

## README.md Template

**Key placeholders to replace after CFN deployment:**
- `{STACK_NAME}` — The actual deployed CloudFormation stack name (e.g.,
  `fis-az-power-int-my-cluster-a3x7k2`)
...
**IMPORTANT:** After successful CFN deployment, update all `{STACK_NAME}` placeholders
in the README with the actual stack name. Do NOT leave generic placeholders in the
