...
3. **并行日志收集** — 后台 `kubectl logs -f` 进程同时收集多个应用日志，仅收集常规容器日志（排除 FIS 注入的临时容器）
4. **实时洞察展示** — 每 30 秒：实际错误日志（5 行）+ 按服务分组的分析洞察
5. **全面分析报告** — 错误时间线、模式识别、跨服务关联、恢复分析
...
```
{实验目录}/
├── {时间戳}-app-logs/              # 带时间戳，支持多次运行
│   ├── {服务-1}/
│   │   ├── {应用-1}.log
│   │   └── {应用-2}.log
│   └── {服务-2}/
│       └── {应用-3}.log
└── {时间戳}-app-log-analysis.md
```
