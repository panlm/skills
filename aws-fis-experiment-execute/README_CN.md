...
5. **发现 EKS 应用依赖并启动日志收集** — 遵循 `eks-app-log-analysis` skill（实时模式，步骤 3-4）自动发现依赖受影响 AWS 服务的 EKS Pod，与用户确认后启动后台 `kubectl logs -f`。**此步骤在实验启动前完成**，避免丢失早期日志。如果用户要求前后对比，可选收集 2 分钟基线日志。
6. **强制安全确认** — 展示清晰的影响警告（受影响资源和监控的应用列表），要求用户明确确认后才启动。
7. **启动实验** — 仅在用户明确确认后执行。
8. **监控进度 + 日志洞察** — 每 30-60 秒轮询实验状态，记录每次状态变更和各服务事件的时间戳，通过 `eks-app-log-analysis` 步骤 5 显示各应用错误/警告计数和恢复信号，提醒用户查看 Dashboard。
9. **停止日志收集并分析** — 遵循 `eks-app-log-analysis` 步骤 7-8 终止后台进程，分析错误模式、峰值速率和恢复时间。可选等待 2 分钟收集实验后基线（如用户选择）。
10. **保存结果报告** — 将实验结果写入本地 Markdown 文件（`YYYY-mm-dd-HH-MM-SS-{scenario}-experiment-results.md`），包含**按服务拆分的影响分析**、**应用日志分析**（报告格式定义在 `eks-app-log-analysis` 中）和原始日志文件位置。终端仅打印简要摘要。
...
步骤 4: 从 Stack 输出提取实验模板 ID
         ↓
步骤 5: 发现 EKS 应用 + 启动日志收集 [实验启动前完成]
         ├── 遵循 eks-app-log-analysis skill（实时模式）步骤 3-4
         ├── 默认：立即开始收集
         └── 可选（用户选择）：先收集 2 分钟基线日志
         ↓
步骤 6: 启动实验 [关键 — 需要用户明确确认]
...
步骤 7: 监控实验 + 日志洞察
         ...
         ├── 每次轮询后显示当前状态 + 各应用错误/警告计数（通过 eks-app-log-analysis 步骤 5）
         ├── 记录每次状态变更和 Action 转换的时间戳
         └── 提醒用户：查看 Dashboard
         ↓
步骤 8: 停止日志收集 + 分析
         ├── 遵循 eks-app-log-analysis 步骤 7-8
         ├── 默认：实验结束后立即停止
         └── 可选（用户选择）：等待 2 分钟收集实验后基线
         ↓
步骤 9: 保存结果报告到本地文件 (YYYY-mm-dd-HH-MM-SS-{scenario}-experiment-results.md)
```
...
5. **集成的日志收集和分析。** 应用日志的发现、收集、监控和分析遵循 `eks-app-log-analysis` skill（实时模式）— 不重复实现逻辑。结果报告在同一文档中嵌入应用日志分析。`eks-app-log-analysis` skill 也可独立用于事后分析。
