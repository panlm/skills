...
5. **询问是否收集应用日志** — 默认为否（基础设施模式）。如果用户已在对话中表达了分析日志的意图则自动选是。选是时加载 `eks-app-log-analysis` skill，在实验前发现 EKS 应用并启动后台日志收集。
...
5. **日志收集为可选（默认不收集）。** 实验执行前询问用户是否收集应用日志，默认为否。infra 团队可以快速执行实验而无需 kubectl；应用团队选择收集日志后，自动加载 `eks-app-log-analysis` skill 处理发现、收集和分析。如果用户在对话中已表达日志分析意图则自动启用。`eks-app-log-analysis` skill 也可独立用于事后分析。
