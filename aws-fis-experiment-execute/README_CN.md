...
5. **判断是否收集应用日志** — 针对 pod 实验（`aws:eks:pod-*` actions）自动启用，用户在对话中表达了日志分析意图时也自动启用。其他情况询问用户（默认为否）。选是时加载 `eks-app-log-analysis` skill，在实验前发现 EKS 应用并启动后台日志收集。
6. **强制安全确认** — 展示清晰的影响警告（受影响资源，启用日志收集时包含监控的应用列表），要求用户明确确认后才启动。
7. **启动实验** — 仅在用户明确确认后执行。
8. **监控进度** — 每 30-60 秒轮询实验状态，记录每次状态变更和各服务事件的时间戳。启用日志收集时还显示各应用错误/警告计数和恢复信号。
9. **停止日志收集并分析** — （**仅在启用日志收集时**）遵循 `eks-app-log-analysis` 步骤 7-8 终止后台进程，分析错误模式、峰值速率和恢复时间。
10. **保存结果报告** — 将实验结果写入本地 Markdown 文件，包含**按服务拆分的影响分析**，启用日志收集时还包含**应用日志分析**。终端仅打印简要摘要。
...
步骤 4: 从 Stack 输出提取实验模板 ID
         ↓
步骤 4.5: 判断是否收集应用日志
         ├── 自动启用：pod 实验 (aws:eks:pod-*) 或用户表达了日志意图
         ├── 其他情况询问（默认：否 → 跳至步骤 6）
         └── 是 → 继续步骤 5
         ↓ (如选是)
...
步骤 6: 启动实验 [关键 — 需要用户明确确认]
         ...
         └── 用户拒绝 → 中止（如有日志收集则先清理）
         ↓
步骤 7: 监控实验（启用日志收集时含日志洞察）
         ├── 前 5 分钟每 30 秒轮询，之后每 60 秒
         ├── 记录每次状态变更和 Action 转换的时间戳
         ├── 如收集日志：显示各应用错误/警告计数
         └── 提醒用户：查看 Dashboard
         ↓ (如收集日志)
步骤 8: 停止日志收集 + 分析（通过 eks-app-log-analysis 步骤 7-8）
         ↓
...
4. **应用发现在实验启动前完成。** 选择收集日志时，EKS 应用依赖在实验开始前发现，日志收集也在实验前启动。这防止实验开始后才找应用，导致早期日志被轮转或覆盖而丢失。

5. **日志收集可选（pod 实验自动启用）。** 针对 `aws:eks:pod-*` actions，日志收集自动启用 — pod 实验本质上需要应用日志分析。其他实验询问用户（默认否）。infra 团队无需 kubectl 即可快速执行；应用团队和 pod 实验通过 `eks-app-log-analysis` 获得完整日志分析。该 skill 也可独立用于事后分析。
