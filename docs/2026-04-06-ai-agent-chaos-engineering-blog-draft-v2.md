...
3. **展示影响范围，要求用户明确确认** —— 显示目标资源、影响区域、预计时长
4. 用户确认后才启动实验
5. 轮询实验状态，提醒用户查看 CloudWatch Dashboard
6. 生成结果报告：按服务分组的影响分析，包括时间线、观察和关键发现
...
4. 自动收集相关应用的 kubectl logs（实时模式用 `kubectl logs -f`，事后模式优先查询 CloudWatch Logs Insights）
5. 生成应用层分析报告：
   - 错误时间线（与 FIS 事件对齐）
   - 错误模式统计
   - 恢复时间分析
